import React, { Fragment, useState } from 'react';
import { Link,useNavigate } from 'react-router-dom';
const Register = ({ setAuth }) => {
    const navigate = useNavigate();
    const [inputs, setInputs] = useState({
        username: "",
        password: "",
        name: "" ,
        no_phone: "",
        email: "",
        type: "student"
    })

    const { username, password, name, no_phone, email, type } = inputs
    const onChange = (e) => {
        setInputs({ ...inputs, [e.target.name]: e.target.value })
    }

    const onSubmitForm = async(e) => {
        e.preventDefault()
        try {
            const body = { username, password, name, no_phone, email, type }
            console.log(JSON.stringify(body))
            const response = await fetch('http://localhost:27017/auth/register', {
                method: "POST",
                headers: {
                    "Content-Type": "application/json"
                },
                body: JSON.stringify(body)
            })

            const parseRes = await response.json()
            localStorage.setItem("token", parseRes.token)
            console.log(parseRes)
            // setAuth(true)
            if (parseRes.token) {
                // Assuming a successful response contains a token
                localStorage.setItem("token", parseRes.token); // Save the token locally
                // setAuth(true); // Set user as authenticated
                navigate('/login'); // Redirect to the dashboard
            } else {
                setAuth(false);
            }
        } catch (error) {
            console.error(error.message)
        }
    }
  return (
    <Fragment>
      <h1>Register</h1>
      <form onSubmit={onSubmitForm}>
        <input type="text" name="name" placeholder='name' className="form-control my-3" value={name} onChange={e => onChange(e)}/> 
        <input type="email" name="email" placeholder='email' className="form-control my-3" value={email} onChange={e => onChange(e)}/> 
        <input type="text" name="no_phone" placeholder='no_phone' className="form-control my-3" value={no_phone} onChange={e => onChange(e)}/> 
        <select
            name="type"
            className="form-control my-3"
            value={type}
            onChange={e => onChange(e)}
        >
            <option value="student">Student</option>
            <option value="admin">Admin</option>
            <option value="lecturer">Lecturer</option>
        </select>
        <input type="text" name="username" placeholder='Username' className="form-control my-3" value={username} onChange={e => onChange(e)}/> 
        <input type="passwword" name="password" placeholder='Password' className="form-control my-3" value={password} onChange={e => onChange(e)}/> 
        <button  className='btn btn-success btn-block'>Register</button>
      </form>
      <Link to="/login">Login</Link>
    </Fragment>
  )
}

export default Register

